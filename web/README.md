...
owns a running [`MacBrain`](../MAC_BRAIN/runtime.py) and serves a browser UI for live
...
```bash
.venv/bin/python -m web.server --host 127.0.0.1 --port 8080 --store novi_web.db --tick 0.8
```
...
| Script | Description |
| --- | --- |
| [`scripts/mac-web.sh`](../scripts/mac-web.sh) | **Start the web app.** `python -m web.server` with sensible defaults; env overrides `NOVI_HOST/PORT/STORE/TICK`. |
| [`scripts/mac-brain.sh`](../scripts/mac-brain.sh) | **Start the interactive CLI brain** (`python -m MAC_BRAIN.cli`). Pass CLI flags through, e.g. `--live`. |
| [`scripts/mac-brain-demo.sh`](../scripts/mac-brain-demo.sh) | **Start a canned demo.** Modes: `live` (webcam + neural vision + STT + router reasoning + TTS), `neural`/`image` (MPS object detection on a static image), `hear` (offline deterministic speech), `quick` (deterministic brain, no camera/mic/model), or pass through anything else to the CLI. |
| [`scripts/mac-brain-test.sh`](../scripts/mac-brain-test.sh) | **Run the Brain test suite** and write a summarized JSON result to `mac_test_results/MAC_BRAIN/<timestamp>/`. |
...
| `scripts/mac/collect-evidence.sh` | Snapshot the latest run into `IMPLEMENTATION_PLAN/EVIDENCE/mac/<timestamp>/`. |
