...
| 18. Prepare Qwen3.8-27B into AirLLM shards | BLOCKED (environment) | checkpoint is 55.6 GB (HF tree); source + shards + reserve ≈ 112 GB required vs 99 GiB free after round-4 cleanup (was 58 GiB) — `check_disk_capacity` still refuses with typed `StorageCapacityError`. Also blocked on the Step 17 Transformers conflict + Mac MLX path architecture limits (upstream AirLLM). Backend remains an implemented, platform-blocked provider for the 27B (§17); the GENERIC AirLLM path runs on Mac via compatible models (see Step 19–22/34 rows) |
...
| 30. Long-duration soak tests | HARNESS + REAL SOAK DONE (raw + routed) | `soak.py` (CI-safe); raw-backend soak 27.85 s avg (σ≈0.02 s, 0 failures); **routed soak through the full runtime**: 4 requests, all routed to `backend=airllm`, 31.3 s avg, 0 failures (`route_soak` block in `tinyllama-1.1b.json`); 1h/4h/8h/24h require target hardware |
...
| R4. Generic AirLLM everywhere (user directive 2026-08-30) | DONE + execution-verified | per-platform `architecture_compatibility` pre-check (Mac/MLX: LlamaForCausalLM supported; Qwen3/Qwen3_5 unsupported; unknown never promoted); `AirLLMBackend.check_model_compatibility` gates validate + router; CUDA-only VRAM gate; `tinyllama-1.1b` registered as documented Mac-verified reference; **end-to-end route proof: router selected `backend=airllm` and completed real MLX-streamed generation (ok=True, 0 failures/fallbacks)** + routed soak 4/4 airllm, 0 failures — `benchmarks/airllm-mac/tinyllama-1.1b.json` |
