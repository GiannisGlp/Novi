...
31. **No resource-aware behavioral adaptation fed by real telemetry** (battery/compute/thermal/network) — MultiSpeedRuntime has resource modes but MacBrain never sets them. **✅ Partially FIXED**: `MacBrain.step()` now maps the FailureHandler degraded mode to resource modes (FULL/REACTIVE_ONLY/DEGRADED/SAFE_MINIMUM) each cycle; real telemetry feed remains.
