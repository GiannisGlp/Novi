...
from brain.memory_hardening import (
    CONFLICTED,
    DIRECT_SENSOR,
    NO_RESULT,
    OBSERVED,
...
from brain.models import DeterministicSTTProvider
from brain.engine import MacBrain
...
class SchemaVersionCompatibilityTests(unittest.TestCase):
    ...
    def test_new_store_records_current_version(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            from brain.storage import SCHEMA_MAJOR, SCHEMA_MINOR
    ...
    def test_reopened_store_preserves_version(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIsNotNone(row)
            from brain.storage import SCHEMA_MAJOR
            self.assertEqual(row["major"], SCHEMA_MAJOR)
            s2.close()
