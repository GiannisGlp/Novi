...
from brain.io import CameraFrame, VirtualBody
...
class MacBrainTests(unittest.TestCase):
    def test_brain_composes_existing_brain_runtime(self) -> None:
        camera = FakeCamera()
    ...
    def test_brain_emits_observable_events(self) -> None:
        brain = MacBrain(camera=FakeCamera())
        brain.start()
        brain.step()
        brain.stop()
        event_types = [event["event_type"] for event in brain.events]
        self.assertIn("brain.started", event_types)
        ...
        self.assertIn("brain.stopped", event_types)
