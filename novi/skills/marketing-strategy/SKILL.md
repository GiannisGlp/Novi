...
license: MIT
kind: instruction
triggers: marketing, positioning, go-to-market, launch-plan, competitor, sales-enablement
metadata:
  origin: davila7/claude-code-templates (MIT), author Alireza Rezvani
  upstream_name: marketing-strategy-pmm
---

# Novi usage

Ported from an MIT-licensed skill catalog and adapted for Novi: frontmatter
extended with Novi's `kind`/`triggers` contract. Apply when a matched message
needs it; the brain's FORBIDDEN guard and dialogue rules run AFTER any text
