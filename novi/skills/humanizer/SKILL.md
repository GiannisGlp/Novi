...
license: MIT
kind: instruction
triggers: humanize, rewrite, natural, sound-human
metadata:
  version: "2.11.2"
  origin: agent-skills catalog (.dsh/skills/humanizer), MIT, adapted for Novi
---

# Novi usage

Novi applies this skill as a rewrite pass over its own draft replies when the
local LLM transport is available. The brain's FORBIDDEN guard and all dialogue
rules run AFTER this rewrite; pattern 20 (chatbot text left in the answer)
is exactly what the guard independently enforces.
