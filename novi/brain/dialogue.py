...
_META_REFERENTIAL = [
    ...
    for p in (
        ...
        # qwen3 reasoning remnants that leak past `think` handling — the model
        # narrates its own answer-construction instead of answering.
        r"\bdouble[- ]checking\b",
        r"\b(?:possible|potential) response:?\b",
        r"\bthe response should be\b",
        r"\b(?:my|the) (?:final|best) response (?:should|is)\b",
        r"\bi should say\b",
        r"\bi'?ll say\b",
        ...
        r"\b(?:an|a) (?:appropriate|better|good) (?:response|answer|reply)\b",
        r"\balternatively,? since (?:the|i|we)\b",
        r"\bthis should work\b",
        r"\blet me put that together\b",
        r"\bi'?ll go with\b",
        r"\b(?:i|we) should make sure\b",
        r"\b(?:that'?s|this is) (?:a )?good (?:answer|response|plan)\b",
        r"\bno need for (?:extra )?fluff\b",
        r"\bfits? (?:this|the) (?:request|task|prompt)\b",
        r"\bto (?:summarize|wrap up)\b",
        r"\bchecking (?:for|that|the)\b",
        r"\bthe user (?:wants?|needs?|is asking)\b",
        r"\bthe user'?s last (?:input|message|question|line)\b",
        r"\bwait,? the user\b",
        r"\blet me check the (?:traits|facts|context|situation)\b",
        r"\bso i should (?:respond|answer|say)\b",
        r"\bi should (?:respond|answer|say) with\b",
        r"\bcheck(?:ing)? the traits\b",
        r"\bnovi'?s traits\b",
        r"\bi need to (?:make sure|check|see if)\b",
        r"\b(?:let me|i should) (?:start|begin) (?:by|with)\b",
        r"\b(?:make|keep) (?:sure|it) (?:that|warm|friendly|brief)\b",
        r"\b(?:that|this) (?:works|should work) (?:for|perfectly)\b",
        r"\bi(?: will|')? (?:go|move) (?:with|forward)\b",
...
_META_OPENER = [
    ...
    for p in (
        ...
        r"^(?:wait|alternatively|anyway|hmm|actually)[,.]?\s",
        r"^(?:the user|they)'?s last (?:input|message|question)\s",
        r"^(?:so|then) i should (?:respond|answer|say)\s",
        r"^(?:first|firstly),? (?:the|i|we)\s",
        r"^let me (?:check|start|begin|see)\s",
    )
]
