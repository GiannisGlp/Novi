...
def run() -> int:
    ...
    if r["compute_rail_current_a_min"] < r["compute_power_w_test_max"] * 1.25 / r["compute_rail_voltage_v_nominal"]:
    ...
    expected = {"S0": (2, 5, 1), "S1": (5, 10, 1), "S2": (25, 50, 2), "S3": (80, 120, 2), "S4": (500, 1000, 1)}
    for name, (p99, maximum, depth) in expected.items():
        cls = classes[name]
        if cls["p99_latency_ms"] != p99 or cls["max_latency_ms"] != maximum or cls["queue_depth"] != depth:
            failures.append(f"{name} latency/queue budget changed unexpectedly")
